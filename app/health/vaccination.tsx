...
export default function VaccinationScreen() {
  ...
  const { animals, vaccinations, addVaccination } = useStore();
  ...
  const [nextDueDate, setNextDueDate] = useState(new Date(new Date().setMonth(new Date().getMonth() + 6)));
  ...
  const handleSave = async () => {
    if (!animalId || !vaccineName.trim()) {
      Toast.show({ type: 'error', text1: 'Validation Error', text2: 'Animal and vaccine name are required.' });
      return;
    }
    ...
    Toast.show({ type: 'success', text1: 'Saved', text2: 'Vaccination record added.' });
      ...
      <AppBackground>
        ...
        <View style={s.card}>
          ...
          <View style={s.inputGroup}>
            ...
            <TextInput style={s.input} value={vaccineName} onChangeText={setVaccineName} placeholder="e.g. FMD" />
          </View>
          ...
          <View style={s.inputGroup}>
            ...
            <TextInput style={s.input} value={administeredBy} onChangeText={setAdministeredBy} placeholder="e.g. Dr. Ijaz" />
          </View>
            ...
            return (
              <View style={s.listCard}>
                <View style={s.listCardHead}>
                  ...
                  <Text style={[s.listCardStatus, { flexShrink: 0 }]}>{item.vaccine_name}</Text>
                </View>
                <View style={s.listCardBody}>
                  ...
                  <View style={s.gridRow}>
                    ...
                    <Text style={[s.gridValue, { color: Colors.warning }]}>{item.next_due_date}</Text>
                  </View>
                </View>
              </View>
            );
...
const s = StyleSheet.create({
  ...
  listCardStatus: { fontSize: 15, fontWeight: '800', color: '#EA580C' },
